...
public class Bytes {
  public static int sizeInBytes(int num, int bitwidth) {
    return (int) Math.ceil((num * bitwidth) / 8D);
  }
  ...
  public static void writeChar(char value, byte[] bytes, int offset) {
    bytes[offset + 1] = (byte) (value >>> 8);
    bytes[offset] = (byte) value;
  }
  ...
  public static char readChar(byte[] bytes, int offset) {
    return ((char) (((bytes[offset + 1] & 255) << 8) +
                  (bytes[offset] & 255)));
  }
  ...
  public static void readFully(byte[] buffer, InputStream is, int len) throws IOException {
    int n = 0;
    while (n < len) {
      int count = is.read(buffer, n, len - n);
      if (count < 0)
        throw new EOFException();
      n += count;
    }
