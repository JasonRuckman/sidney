 ...
 * Spark application.
 */
class CustomKryoSerializer(conf: SparkConf)
  ...
  def newKryo(): Kryo = {
    ...
    for (cls <- CustomKryoSerializer.toRegister) {
      kryo.register(cls)
    }

    // For results returned by asJavaIterable. See JavaIterableWrapperSerializer.
    kryo.register(CustomJavaIterableWrapperSerializer.wrapperClass, new CustomJavaIterableWrapperSerializer)
    ...
    try {
      ...
      userRegistrator
        .map(Class.forName(_, true, classLoader).newInstance().asInstanceOf[CustomKryoRegistrator])
  ...
  override def newInstance(): SerializerInstance = {
    new CustomKryoSerializerInstance(this)
  }
}

private[spark]
class CustomKryoSerializationStream(kryo: Kryo, outStream: OutputStream) extends SerializationStream {
...
class CustomKryoDeserializationStream(kryo: Kryo, inStream: InputStream) extends DeserializationStream {
...
class CustomKryoSerializerInstance(ks: CustomKryoSerializer) extends SerializerInstance {
  ...
  override def serializeStream(s: OutputStream): SerializationStream = {
    new CustomKryoSerializationStream(kryo, s)
  }

  override def deserializeStream(s: InputStream): DeserializationStream = {
    new CustomKryoDeserializationStream(kryo, s)
  }
}
 ...
 */
private class CustomJavaIterableWrapperSerializer
  extends com.esotericsoftware.kryo.Serializer[java.lang.Iterable[_]] {

  import org.apache.spark.serializer.CustomJavaIterableWrapperSerializer._
...
private object CustomJavaIterableWrapperSerializer extends Logging {
