...
public abstract class AbstractBenchmark<T> {
  ...
  protected List<T> doReadSidney(byte[] arr) {
    ...
    reader.open(new LZ4BlockInputStream(new ByteArrayInputStream(arr)));
  ...
  protected List<T> doUnsafeReadSidney() {
    ...
    reader.open(new LZ4BlockInputStream(new ByteArrayInputStream(unsafeSidneyData)));
    List<T> list = reader.readAll();
    reader.close();
    return list;
  }
  ...
  protected List<T> doReadJackson() {
    try {
      return mapper().readValue(new LZ4BlockInputStream(new ByteArrayInputStream(getSerializedJacksonData())), List.class);
    } catch (IOException e) {
  ...
  protected List<T> doReadKryo(byte[] arr) {
    Input input = input = new Input(new LZ4BlockInputStream(new ByteArrayInputStream(arr)));
  ...
  private void initializeKryo() {
    try {
      ...
      LZ4BlockOutputStream gzos = new LZ4BlockOutputStream(outputStream);
      Output o = output();
